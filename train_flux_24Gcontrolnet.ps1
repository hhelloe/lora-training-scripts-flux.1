...
#训练模式(Lora、db、sdxl_lora、Sdxl_db、sdxl_cn3l、stable_cascade_db、stable_cascade_lora、controlnet、hunyuan_lora、hunyuan_db、sd3_db、sd3_lora、flux_lora、flux_db、flux_controlnet)
$train_mode = "flux_controlnet"
...
$train_data_dir = "./train/dsine/0414" # train dataset path | 训练数据集路径
...
$max_train_epoches = 24 # max train epoches | 最大训练 epoch
$save_every_n_epochs = 12 # save every n epochs | 每 N 个 epoch 保存一次
...
$network_dim = 4 # network dim | 常用 4~128，不是越大越好
...
$lr_decay_steps = 0 # decay steps | 学习率衰减步数，仅在 lr_scheduler 为warmup_stable_decay时 需要填写，一般是10%总步数
...
# Adopt,PGSD
# DeMo, ProdigyPlusScheduleFree, ForeachDelayedPSGD
...
$prodigy_steps = 100 #prodigy调整步数，最好设置为训练总步数的1/4
...
$output_name = "controlnetl_sdxl_depth_v2" # output model name | 模型保存名称
...
$sample_prompts = "./toml/controlnet_sdxl" #prompt文件路径
...
$x_mod_dim = 0
$emb_dims = "0,0,0,0,0,0" # 6 dims for context_embedder, t_embedder, x_embedder, y_embedder, final_layer_adaLN_modulation, final_layer_linear
...
$controlnet_model_name_or_path = "" #控制网络模型路径
...
if ($optimizer_type -ieq "ForeachDelayedPSGD") {
  [void]$ext_args.Add("--optimizer_type=heavyball.ForeachDelayedPSGD")
  [void]$ext_args.Add("--optimizer_args")
  [void]$ext_args.Add("q_dtype=$mixed_precision")
  [void]$ext_args.Add("storage_dtype=$save_precision")
}
