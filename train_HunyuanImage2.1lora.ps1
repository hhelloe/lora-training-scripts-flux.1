...
#训练模式(Lora、db、sdxl_lora、Sdxl_db、sdxl_cn3l、stable_cascade_db、stable_cascade_lora、controlnet、sd3_db、hunyuan_image_lora)
$train_mode = "hunyuan_image_lora"

# Train data path | 设置训练用模型、图片
$pretrained_model = "./Stable-diffusion/dit/hunyuanimage2.1.safetensors" # base model path | 底模路径
$vae = "./Stable-diffusion/split_files/vae/hunyuan_image_2.1_vae_fp16.safetensors"
...
$blocks_to_swap = 16 # 交换的块数，默认为6

#flux 相关参数
$ae = $vae
$timestep_sampling = "qinglong_flux" # 时间步采样方法，可选 sd3用"sigma"、普通DDPM用"uniform" 或 flux用"sigmoid" 或者 "shift". shift需要修改discarete_flow_shift的参数
...
# hunyuanimage2.1相关参数
$text_encoder="./Stable-diffusion/split_files/text_encoders/qwen_2.5_vl_7b.safetensors"
$byt5="./Stable-diffusion/split_files/text_encoders/byt5_small_glyphxl_fp16.safetensors"
$fp8_scaled = 1
$fp8_vl = 0
$vae_enable_tiling = 0
...
$output_name = "hunyuan-image-qinglong" # output model name | 模型保存名称
...
if ($train_mode -ilike "hunyuan_image*") {
  $laungh_script = "hunyuan_image_" + $laungh_script
  $network_module = $network_module + "_hunyuan_image"
  $train_unet_only = 1
  ...
  if ($vae_enable_tiling) {
    [void]$ext_args.Add("--vae_enable_tiling")
  }
  if ($cache_text_encoder_outputs -ne 0) { 
    [void]$ext_args.Add("--cache_text_encoder_outputs")
    if ($cache_text_encoder_outputs_to_disk -ne 0) { 
